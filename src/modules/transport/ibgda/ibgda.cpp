...
static void print_ibgda_ep(struct ibgda_ep *ep, int offset, int log_level) {
  TRACE(log_level,
        ...
        "ib_qp=%p, wq_mobject=%p, dbr_mobject=%p, uar_mobject=%p, "
        "wq_offset=%ld, dbr_offset=%ld, send_cq=%p, recv_cq=%p, ah=%p.",
        offset, ep->qp_type, ep->qpn, ep->portid, ep->ib_qp, ep->wq_mobject,
        ep->dbr_mobject, ep->uar_mobject, ep->wq_offset, ep->dbr_offset,
        ep->send_cq, ep->recv_cq, ep->ah);
  return;
}

struct ibgda_mem_handle {
  struct nvshmemt_ib_common_mem_handle
      dev_mem_handles[NVSHMEMI_IBGDA_MAX_DEVICES_PER_PE];
  int num_devs;
};
...
static void print_ibgda_mem_handle(struct ibgda_mem_handle *handle,
  ...
  for (int i = 0; i < handle->num_devs; ++i) {
    struct nvshmemt_ib_common_mem_handle *cur = handle->dev_mem_handles + i;
    TRACE(log_level,
          "print_ibgda_mem_handle: num_devs:%d, idx=%d, mr=%p, buf=%p, fd=%d, "
          "lkey=%u, rkey=%u, local_only=%d.",
          handle->num_devs, i, cur->mr, cur->buf, cur->fd, cur->lkey, cur->rkey,
          cur->local_only);
  }
  return;
}
...
static void print_ibgda_rc_handle(struct ibgda_rc_handle *handle, int num,
  ...
  for (int i = 0; i < num; ++i) {
    struct ibgda_rc_handle *cur = handle + i;
    TRACE(log_level,
          "print_ibgda_rc_handle: pe:%d, offset:%d, qpn=%u, lid=%u, spn=%lx, "
          "iid=%lx.",
          pe, i, cur->qpn, cur->lid, cur->spn, cur->iid);
  }
  return;
}
...
struct ibgda_device_local_only_mhandle_cache {
  ...
  void *dev_ptr; // Ptr to GPU buffer that contains a copy of this mhandle. CPU
                 // cannot dereference.
};

// CPU cannot dereference next
static std::vector<struct ibgda_device_local_only_mhandle_cache>
    ibgda_device_local_only_mhandles;
...
void *ibv_handle = NULL;
pthread_mutex_t ibv_handle_lock = PTHREAD_MUTEX_INITIALIZER;
// count of references only getting success return from
// nvshmemt_ibv_ftable_init() call.
int ibv_handle_refcount = 0;
...
static void print_guid(const unsigned char *idv_gid, int log_level) {
  TRACE(log_level,
        ...
        idv_gid[3], idv_gid[2], idv_gid[1],
        idv_gid[0],             // First 4 bytes (Little-Endian)
        idv_gid[5], idv_gid[4], // Next 2 bytes
        idv_gid[7], idv_gid[6], // Next 2 bytes
        idv_gid[8], idv_gid[9], // Next 2 bytes
        idv_gid[10], idv_gid[11], idv_gid[12], idv_gid[13], idv_gid[14],
        idv_gid[15]);
}
...
ibgda_parse_qp_map_by(nvshmemi_ibgda_device_qp_map_type_t *out_map_by,
  ...
  std::for_each(req.begin(), req.end(),
                [](decltype(*req.begin()) &c) { c = ::tolower(c); });
...
static int get_pci_path(int dev, char **pci_path, nvshmem_transport_t t) {
  ...
  nvshmemt_ibgda_state_t *ibgda_state =
      (nvshmemt_ibgda_state_t *)transport->state;
  int dev_id = ibgda_state->dev_ids[dev];
  const char *ib_name =
      (const char *)((struct ibgda_device *)ibgda_state->devices)[dev_id]
          .dev->name;
...
int nvshmemt_ibgda_get_mem_handle(nvshmem_mem_handle_t *mem_handle,
  ...
  nvshmemt_ibgda_state_t *ibgda_state =
      (nvshmemt_ibgda_state_t *)transport->state;
  TRACE(ibgda_state->log_level, "nvshmemt_ibgda_get_mem_handle enter");
  ...
  ibgda_device_state =
      (nvshmemi_ibgda_device_state_t *)transport->type_specific_shared_state;
  ...
  for (int i = 0; i < n_devs_selected; ++i) {
    ...
    nvshmem_mem_handle_t *dev_handle =
        (nvshmem_mem_handle_t *)&handle->dev_mem_handles[i];
    ...
    print_nvshmemt_ib_common_mem_handle(
        (struct nvshmemt_ib_common_mem_handle *)dev_handle,
        ibgda_state->log_level);
  }
  ...
  if (local_only) {
    ...
    NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY,
                          out, "cudaMalloc failed.\n");

    device_mhandle_h->start = (uint64_t)buf;
    device_mhandle_h->end = (uint64_t)buf + length - 1;
    device_mhandle_h->is_sysmem_scope =
        (buf_attributes.type != cudaMemoryTypeDevice);
    ...
    if (ibgda_device_local_only_mhandles.empty()) {
      ...
      mhandle_gpu_ptr =
          (void *)((uintptr_t)last_mhandle_cache->dev_ptr +
      ...
      NVSHMEMI_NE_ERROR_JMP(
          status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_add_device_remote_mem_handles: length=%lu; "
          "log2_cumem_granularity=%lu; num_elements=%lu; n_devs_selected=%d.",
          length, transport->log2_cumem_granularity, num_elements,
          n_devs_selected);
    while (num_elements > 0) {
      for (int i = 0; i < n_devs_selected; i++) {
        device_lkey = htobe32(handle->dev_mem_handles[i].lkey);
        TRACE(ibgda_state->log_level,
              "nvshmemt_ibgda_add_device_remote_mem_handles: idx=%d, "
              "mem_handle lkey=%u; device_lkey=%u.",
              i, handle->dev_mem_handles[i].lkey, device_lkey);
    ...
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_add_device_remote_mem_handles: num_lkeys=%lu.",
          num_lkeys);
    ...
    if (num_lkeys > NVSHMEMI_IBGDA_MAX_CONST_LKEYS) {
      size_t lkeys_array_size = sizeof(nvshmemi_ibgda_device_key_t) *
                                (num_lkeys - NVSHMEMI_IBGDA_MAX_CONST_LKEYS);
      ...
      status = cudaMemcpyAsync(ibgda_device_lkeys_d, (const void *)data_ptr,
                               lkeys_array_size, cudaMemcpyHostToDevice,
                               ibgda_state->my_stream);
      NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                            "Copying lkeys to GPU memory failed.\n");
    }
    ibgda_device_state->globalmem.lkeys =
        (nvshmemi_ibgda_device_key_t *)ibgda_device_lkeys_d;
  }
...
out:
  ...
  print_ibgda_mem_handle(handle, ibgda_state->log_level);
  TRACE(ibgda_state->log_level, "end nvshmemt_ibgda_get_mem_handle");
  return status;
}
...
int nvshmemt_ibgda_get_mem_handle_v2(nvshmem_mem_handle_t *mem_handle,
  ...
  nvshmemt_ibgda_state_t *ibgda_state =
      (nvshmemt_ibgda_state_t *)transport->state;
  TRACE(ibgda_state->log_level, "enter nvshmemt_ibgda_get_mem_handle_v2");
...
out:
  ...
  print_ibgda_mem_handle(handle, ibgda_state->log_level);
  TRACE(ibgda_state->log_level, "end nvshmemt_ibgda_get_mem_handle_v2");
  return status;
}
...
static int ibgda_gpu_mem_alloc(struct ibgda_mem_object **pmobject, size_t size,
  ...
  status =
      CUPFN(ibgda_cuda_syms,
            cuPointerSetAttribute(&attr_val, CU_POINTER_ATTRIBUTE_SYNC_MEMOPS,
                                  (CUdeviceptr)ptr));
...
static int ibgda_host_mem_alloc(struct ibgda_mem_object **pmobject, size_t size,
  ...
  if (gpu_mapping) {
    status = cudaHostRegister(
        ptr, size, cudaHostRegisterPortable | cudaHostRegisterMapped);
    NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
...
static int ibgda_nic_mem_gpu_map(struct ibgda_mem_object **pmobject,
  ...
  status = cudaHostRegister(uar->reg_addr, size,
                            cudaHostRegisterPortable | cudaHostRegisterMapped |
                                cudaHostRegisterIoMemory);
  if (status != cudaSuccess) {
    NVSHMEMI_WARN_PRINT("cudaHostRegister with IoMemory failed with error=%d. "
                        "We may need to use a fallback "
                        "path.\n",
                        status);
    status = NVSHMEMX_ERROR_INTERNAL;
    goto out;
...
static int ibgda_create_cq(struct ibgda_cq **pgcq, struct ibgda_device *device,
  ...
  struct mlx5dv_devx_umem *dbr_umem =
      device->cq_shared_object.dbr_mobject->umem;
  ...
  NVSHMEMI_NULL_ERROR_JMP(uar, status, ENOMEM, out,
                          "cannot allocate mlx5dv_devx_uar\n");
  ...
  DEVX_SET(cqc, cq_context, log_page_size,
           IBGDA_GPAGE_BITS - MLX5_ADAPTER_PAGE_SHIFT);
  DEVX_SET64(cqc, cq_context, dbr_addr, dbr_offset); // DBR offset

  gcq->devx_cq = mlx5dv_devx_obj_create(context, cmd_in, sizeof(cmd_in),
  ...
  device->cq_shared_object.cur_cq_off +=
      device->cq_shared_object.cq_buf_size_per_cq;
...
static void ibgda_get_device_cq(nvshmemi_ibgda_device_cq_t *dev_cq,
  ...
  dev_cq->cqe =
      (void *)((uintptr_t)cq->cq_mobject->aligned.gpu_ptr + cq->cq_offset);
...
static int ibgda_qp_rst2init(struct ibgda_ep *ep,
  ...
  if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_DCI) {
    ...
    /* Currently, NVSHMEM APIs only support atomics up to 64. This field can be
     * updated to support atomics up to 256 bytes. */
  ...
  status = mlx5dv_devx_obj_modify(ep->devx_qp, cmd_in, sizeof(cmd_in), cmd_out,
                                  sizeof(cmd_out));
...
static int ibgda_rc_init2rtr(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  TRACE(ibgda_state->log_level, "ibgdr_rc_init2rtr enter");
  int status = 0;
  ...
  DEVX_SET(qpc, qpc, log_rra_max,
           IBGDA_ILOG2_OR0(device->device_attr.max_qp_rd_atom));
  ...
  if (port_attr->link_layer == IBV_LINK_LAYER_INFINIBAND) {
    ...
    const char *nic_device_name =
        ftable.get_device_name(device->context->device);
    int roce_version = 0;
    ...
    ftable.query_gid(device->context, portid,
                     device->gid_info[portid - 1].local_gid_index,
                     (ibv_gid *)&device->gid_info[portid - 1].local_gid);
    print_guid(device->gid_info[portid - 1].local_gid.raw,
               ibgda_state->log_level);

    status = ib_roce_get_version_num(
        nic_device_name, portid, device->gid_info[portid - 1].local_gid_index,
        &roce_version);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    TRACE(ibgda_state->log_level, "ibgdr_rc_init2rtr: spn=%lx; iid=%lx",
          peer_ep_handle->spn, peer_ep_handle->iid);
    ...
    ah_attr.dlid =
        port_attr->lid | (roce_version == 1 ? IBGDA_ROCE_V1_UDP_SPORT_BASE
                                            : IBGDA_ROCE_V2_UDP_SPORT_BASE);

    ah = ftable.create_ah(device->pd, &ah_attr);
    NVSHMEMI_NULL_ERROR_JMP(ah, status, NVSHMEMX_ERROR_INTERNAL, out,
                            "Unable to create ah.\n");
...
out:
  TRACE(ibgda_state->log_level, "ibgdr_rc_init2rtr exit with code %d", status);
  return status;
}
...
static int ibgda_destroy_internal_buffer(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  if (internal_buf_mhandle) {
    nvshmemt_ib_common_release_mem_handle(
        &ftable, (nvshmem_mem_handle_t *)internal_buf_mhandle,
        ibgda_state->log_level);
...
ibgda_create_internal_buffer(struct ibgda_internal_buffer *internal_buf,
  ...
  size_t size_per_dci =
      NVSHMEMI_IBGDA_IBUF_SLOT_SIZE *
      (ibgda_num_fetch_slots_per_dci + IBGDA_IBUF_RESERVED_SLOTS);
  size_t size_per_rc =
      NVSHMEMI_IBGDA_IBUF_SLOT_SIZE *
      (ibgda_num_fetch_slots_per_rc + IBGDA_IBUF_RESERVED_SLOTS);
  size_t buf_size = (size_per_dci * device->dci.num_eps) +
                    (size_per_rc * device->rc.num_eps_per_pe * n_pes);

  status = ibgda_gpu_mem_alloc(&internal_buf_mobject, buf_size,
                               IBGDA_GPAGE_SIZE, false);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot allocate internal buffer.\n");

  internal_buf_mhandle = (struct nvshmemt_ib_common_mem_handle *)calloc(
      1, sizeof(*internal_buf_mhandle));
  NVSHMEMI_NULL_ERROR_JMP(internal_buf_mhandle, status,
  ...
  status = nvshmemt_ib_common_reg_mem_handle(
      ...
      ibgda_state->dmabuf_support_for_data_buffers, ibgda_cuda_syms,
      ibgda_state->log_level, ibgda_state->options->IB_ENABLE_RELAXED_ORDERING);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
static int ibgda_destroy_qp_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  if (device->qp_shared_object.prod_idx_cache)
    free(device->qp_shared_object.prod_idx_cache);
...
static int ibgda_create_cq_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  unsigned int num_cqs =
      device->dci.num_eps + device->rc.num_eps_per_pe * n_pes * 2;
  ...
  status =
      ibgda_mobject_nic_map(cq_mobject, context, IBV_ACCESS_LOCAL_WRITE,
                            ibgda_state->dmabuf_support_for_control_buffers);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot register cq buf.\n");

  // Allocate and map Doorbell Record buffer for all CQs.
  status =
      ibgda_nic_control_alloc(&dbr_mobject, dbr_buf_size, IBGDA_GPAGE_SIZE);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot allocate dbr buf.\n");

  status =
      ibgda_mobject_nic_map(dbr_mobject, context, IBV_ACCESS_LOCAL_WRITE,
                            ibgda_state->dmabuf_support_for_control_buffers);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot register dbr buf.\n");
...
out:
  if (status) {
    if (dbr_mobject) {
      if (dbr_mobject->has_nic_mapping)
        ibgda_mobject_nic_unmap(dbr_mobject);
      ibgda_nic_control_free(dbr_mobject);
    }
    if (cq_mobject) {
      if (cq_mobject->has_nic_mapping)
        ibgda_mobject_nic_unmap(cq_mobject);
...
static int ibgda_create_qp_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  unsigned int num_eps =
      device->dci.num_eps + device->rc.num_eps_per_pe * n_pes;
  ...
  NVSHMEMI_NULL_ERROR_JMP(srq, status, NVSHMEMX_ERROR_INTERNAL, out,
  ...
  srqn = dvsrq.srqn;
  NVSHMEMI_EQ_ERROR_JMP(
      srqn, 0, NVSHMEMX_ERROR_INTERNAL, out,
  ...
  status = ibgda_create_internal_buffer(&device->qp_shared_object.internal_buf,
                                        ibgda_state, device, n_pes);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
  ...
  wq_buf_size_per_qp =
      num_wqebb * MLX5_SEND_WQE_BB * 2; // num_wqebb is always a power of 2
  wq_buf_size = wq_buf_size_per_qp * num_eps;
  status = ibgda_nic_control_alloc(&wq_mobject, wq_buf_size, IBGDA_GPAGE_SIZE);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot allocate wq buf.\n");

  status =
      ibgda_mobject_nic_map(wq_mobject, context, IBV_ACCESS_LOCAL_WRITE,
                            ibgda_state->dmabuf_support_for_control_buffers);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot register wq buf.\n");
  ...
  if (ibgda_nic_handler == IBGDA_NIC_HANDLER_GPU)
    ...
    status = ibgda_host_mem_alloc(&dbr_mobject, dbr_buf_size, IBGDA_GPAGE_SIZE,
                                  true);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "cannot allocate dbr buf.\n");
...
out:
  if (status) {
    ...
    if (wq_mobject) {
      if (wq_mobject->has_nic_mapping)
        ibgda_mobject_nic_unmap(wq_mobject);
    ...
    if (prod_idx_cache)
      free(prod_idx_cache);
    if (prod_idx_snapshot)
      free(prod_idx_snapshot);
  }
  return status;
}
...
static int ibgda_alloc_and_map_qp_uar(struct ibv_context *context,
...
#endif
  {
    ...
    status = mlx5dv_devx_general_cmd(context, cmd_cap_in, sizeof(cmd_cap_in),
  ...
  if (handler == IBGDA_NIC_HANDLER_GPU) {
    ...
    if (status) {
      NVSHMEMI_WARN_PRINT("ibgda_nic_mem_gpu_map failed. We may need to use "
...
static int ibgda_create_qp(struct ibgda_ep **ep_ptr,
  ...
  DEVX_SET(qpc, qp_context, cqn_rcv,
           qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC
               ? recv_cq->cqn
               : device->qp_shared_object.rcqn);
  ...
  DEVX_SET(qpc, qp_context, dbr_umem_valid,
           IBGDA_MLX5_UMEM_VALID_ENABLE); // Enable dbr_umem_id
  DEVX_SET64(qpc, qp_context, dbr_addr,
             dbr_offset); // Offset of dbr_umem_id (behavior changed because of
                          // dbr_umem_valid)
  ...
  if (qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC) {
    ...
    DEVX_SET(qpc, qp_context, log_rq_stride,
             IBGDA_ILOG2(recv_wqe_size) - 4); // max recv wqe size = 16B
  } else {
    DEVX_SET(qpc, qp_context, rq_type,
             IBGDA_SRQ_TYPE_VALUE); // Shared Receive Queue, DC must use this.
    DEVX_SET(qpc, qp_context, srqn_rmpn_xrqn, device->qp_shared_object.srqn);
    DEVX_SET(qpc, qp_context, log_rq_size, 0);
  }

  ep->devx_qp = mlx5dv_devx_obj_create(context, cmd_in, sizeof(cmd_in), cmd_out,
                                       sizeof(cmd_out));
  ...
  device->qp_shared_object.cur_wq_off +=
      device->qp_shared_object.wq_buf_size_per_qp;
...
static int ibgda_create_dct_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  cap = DEVX_ADDR_OF(query_hca_cap_out, cmd_cap_out, capability.cmd_hca_cap);
  hca_support_compact_address_vector =
      DEVX_GET(cmd_hca_cap, cap, compact_address_vector);
...
static int ibgda_create_dct(nvshmemt_ibgda_state_t *ibgda_state,
  ...
  ib_qp_attr.qp_access_flags = IBV_ACCESS_REMOTE_WRITE |
...
static int ibgda_get_dct_handle(struct ibgda_dct_handle *dct_handle,
  ...
  // Don't do htobe32 here as we need to determine whether the ext field should
  // be set or not.
...
static void ibgda_get_device_qp(nvshmemi_ibgda_device_qp_t *dev_qp,
  ...
  dev_qp->tx_wq.wqe = (void *)((uintptr_t)ep->wq_mobject->aligned.gpu_ptr +
                               ep->wq_offset + ep->sq_buf_offset);

  if (ibgda_nic_handler == IBGDA_NIC_HANDLER_GPU) {
    assert(ep->dbr_mobject->has_gpu_mapping);
    dev_qp->tx_wq.dbrec =
        (__be32 *)((uintptr_t)ep->dbr_mobject->aligned.gpu_ptr +
  ...
  if (ep->qp_type == NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC) {
    ...
    dev_qp->rx_wq.wqe = (void *)((uintptr_t)ep->wq_mobject->aligned.gpu_ptr +
                                 ep->wq_offset + ep->rq_buf_offset);
    dev_qp->rx_wq.dbrec =
        (__be32 *)((uintptr_t)ep->dbr_mobject->aligned.gpu_ptr +
                   ep->dbr_offset);
  ...
  ibuf_dci_start =
      (uintptr_t)
          device->qp_shared_object.internal_buf.mem_object->aligned.gpu_ptr;
  ...
  dev_qp->ibuf.lkey =
      htobe32(device->qp_shared_object.internal_buf.mem_handle->lkey);
  dev_qp->ibuf.rkey =
      htobe32(device->qp_shared_object.internal_buf.mem_handle->rkey);
...
static int ibgda_setup_gpu_state(nvshmem_transport_t t) {
  ...
  const size_t prod_idx_offset =
      offsetof(nvshmemi_ibgda_device_qp_management_t, tx_wq.prod_idx);
  const size_t cons_t_offset =
      offsetof(nvshmemi_ibgda_device_qp_management_t, tx_wq.cons_idx);
  const size_t wqe_h_offset =
      offsetof(nvshmemi_ibgda_device_qp_management_t, tx_wq.resv_head);
  const size_t wqe_t_offset =
      offsetof(nvshmemi_ibgda_device_qp_management_t, tx_wq.ready_head);
  const size_t rx_resv_head_offset =
      offsetof(nvshmemi_ibgda_device_qp_management_t, rx_wq.resv_head);
  const size_t rx_cons_offset =
      offsetof(nvshmemi_ibgda_device_qp_management_t, rx_wq.cons_idx);
  ...
  for (int j = 0; j < n_devs_selected; j++) {
    ...
    num_cq_handles +=
        device->dci.num_eps + (device->rc.num_eps_per_pe * (n_pes - 1) * 2);
  ...
  dct_h =
      (nvshmemi_ibgda_device_dct_t *)calloc(num_dct_handles, sizeof(*dct_h));
  ...
  NVSHMEMI_NULL_ERROR_JMP(recv_cq_h, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "recv_cq calloc err.");
  ...
  status = cudaMalloc(&qp_group_switches_d,
                      num_qp_groups * sizeof(*qp_group_switches_d));
  NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
  ...
  for (int i = 0; i < num_dct_handles / n_devs_selected; ++i) {
    for (int j = 0; j < n_devs_selected; j++) {
      ...
      struct ibgda_device *device =
          (struct ibgda_device *)ibgda_state->devices + dev_idx;
      ibgda_get_device_dct(&dct_h[arr_idx], &device->dct.dct_handles[i],
                           device);
    }
  }
  ...
  for (int i = 0; i < num_dci_handles / n_devs_selected; ++i) {
    for (int j = 0; j < n_devs_selected; j++) {
      ...
      if (ibgda_nic_handler == IBGDA_NIC_HANDLER_GPU) {
        ...
        cq_h[cq_idx].prod_idx =
            (uint64_t *)(base_mvars_d_addr + prod_idx_offset);
      } else {
        dci_h[arr_idx].tx_wq.prod_idx = &(
            (uint64_t *)
                device->qp_shared_object.prod_idx_mobject->aligned.gpu_ptr)[i];
    ...
    for (int i = 0; i < num_rc_handles / n_devs_selected; ++i) {
      ...
      for (int j = 0; j < n_devs_selected; j++) {
        ...
        uintptr_t base_mvars_d_addr =
            (uintptr_t)(&rc_d[arr_idx]) + mvars_offset;
        ...
        cq_h[cq_idx].cons_idx = (uint64_t *)(base_mvars_d_addr + cons_t_offset);
        cq_h[cq_idx].resv_head = (uint64_t *)(base_mvars_d_addr + wqe_h_offset);
        cq_h[cq_idx].ready_head =
            (uint64_t *)(base_mvars_d_addr + wqe_t_offset);
        ...
        cq_h[cq_idx].resv_head =
            (uint64_t *)(base_mvars_d_addr + rx_resv_head_offset);
        cq_h[cq_idx].cons_idx =
            (uint64_t *)(base_mvars_d_addr + rx_cons_offset);
        cq_h[cq_idx].qpn = rc_h[arr_idx].qpn;
        cq_h[cq_idx].qp_type = rc_h[arr_idx].qp_type;
        ++cq_idx;
      }
  ...
  cudaMemsetAsync(qp_group_switches_d, 0,
                  num_qp_groups * sizeof(*qp_group_switches_d),
                  ibgda_state->my_stream);
  NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
  ...
  memcpy(ibgda_device_state_h->constmem.dcts, dct_h,
         sizeof(*dct_h) *
             IBGDA_MIN(num_dct_handles, NVSHMEMI_IBGDA_MAX_CONST_DCTS));
  ...
  if (num_dct_non_cache_handles > 0) {
    ...
    status = cudaMemcpyAsync(dct_d, dct_h_ptr,
                             sizeof(*dct_d) * num_dct_non_cache_handles,
                             cudaMemcpyHostToDevice, ibgda_state->my_stream);
    NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "dct copy err.");
  }

  status = cudaMemcpyAsync(dci_d, (const void *)dci_h,
                           sizeof(*dci_h) * num_dci_handles,
                           cudaMemcpyHostToDevice, ibgda_state->my_stream);
  NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                        "dci copy err.");
  ...
  status =
      cudaMemcpyAsync(cq_d, (const void *)cq_h, sizeof(*cq_h) * num_cq_handles,
                      cudaMemcpyHostToDevice, ibgda_state->my_stream);
  NVSHMEMI_NE_ERROR_JMP(status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                        "cq copy err.");
  ...
  ibgda_device_state_h->num_exclusive_dcis =
      num_dci_handles - num_shared_dci_handles;
  ibgda_device_state_h->dci_map_type = dc_map_type;
  ibgda_device_state_h->ndcts_per_pe =
      num_dct_handles / n_devs_selected / n_pes;
  ibgda_device_state_h->num_dct_groups =
      IBGDA_MAX(ibgda_device_state_h->num_exclusive_dcis /
                    (num_dct_handles / n_devs_selected),
                1);
  ibgda_device_state_h->num_rc_per_pe =
      num_rc_handles / n_devs_selected / n_pes;
  ...
  ibgda_device_state_h->use_async_postsend =
      (ibgda_nic_handler == IBGDA_NIC_HANDLER_CPU);
  ibgda_device_state_h->num_devices_initialized = n_devs_selected;
  assert(ibgda_nic_buf_location == IBGDA_MEM_TYPE_GPU ||
         ibgda_nic_buf_location == IBGDA_MEM_TYPE_HOST);
  ibgda_device_state_h->nic_buf_on_gpumem =
...
static bool ibgda_cst_is_required(struct ibgda_device *device,
  ...
  if (CUPFN(ibgda_cuda_syms,
            cuDeviceGetAttribute(
                ...
                dev_id))) {
    NVSHMEMI_WARN_PRINT(
        "Cannot query dev attr. Assuming no GDR write ordering\n");
  } else {
...
int nvshmemt_ibgda_create_qps(nvshmem_transport_t t, int *selected_dev_ids,
  ...
  if (num_selected_devs > NVSHMEMI_IBGDA_MAX_DEVICES_PER_PE) {
    NVSHMEMI_WARN_PRINT(
        "IBGDA only supports %d devices, but the lib has requested %d.\n",
  ...
  if (ibgda_state->selected_dev_ids) {
    NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INVALID_VALUE,
  ...
  if (cudaDeviceGetAttribute(&mtpb, cudaDevAttrMaxThreadsPerBlock,
                             gpu_device_id)) {
    NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                       "query max threads per block fail.");
  }

  if (cudaDeviceGetAttribute(&mpc, cudaDevAttrMultiProcessorCount,
                             gpu_device_id)) {
    NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                       "query mpc count fail.");
  }
  ...
  INFO(ibgda_state->log_level, "IBGDA_RC_MAP_BY is set to %s.",
       options->IBGDA_RC_MAP_BY);

  status = ibgda_parse_qp_map_by(&dc_map_type,
  ...
  INFO(ibgda_state->log_level, "IBGDA_DCI_MAP_BY is set to %s.",
       options->IBGDA_DCI_MAP_BY);
  /* Get shared dev info end */

  /* Allocate global structs start */
  // allocated in create_qps function and deallocated in connect_endpoints
  // function
  ibgda_state->selected_dev_ids =
      (int *)calloc(num_selected_devs, sizeof(*selected_dev_ids));
  NVSHMEMI_NULL_ERROR_JMP(ibgda_state->selected_dev_ids, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "allocation of selected_device_ids failed.\n");

  local_dct_handles = (struct ibgda_dct_handle *)calloc(
      num_dct_eps, sizeof(*local_dct_handles));
  NVSHMEMI_NULL_ERROR_JMP(local_dct_handles, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "allocation of local_dct_handles failed.\n");

  local_rc_handles =
      (struct ibgda_rc_handle *)calloc(num_rc_eps, sizeof(*local_rc_handles));
  NVSHMEMI_NULL_ERROR_JMP(local_rc_handles, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
  ...
  if ((num_dct_eps * num_selected_devs) < 2) {
    NVSHMEMI_WARN_PRINT(
        "NVSHMEM_IBGDA_NUM_DCT < 2 and may impact performance.");
  }
  ...
  for (int i = 0; i < num_selected_devs; i++) {
    if (selected_dev_ids[i] < 0 ||
        selected_dev_ids[i] >= ibgda_state->n_dev_ids) {
    ...
    *out_num_dci_eps = num_dci_eps;
    ...
    NVSHMEMI_NULL_ERROR_JMP(device->dct.dct_handles, status,
                            NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                            "allocation of dct_handles failed.");

    device->dct.eps = (struct ibgda_ep **)calloc(device->dct.num_eps,
                                                 sizeof(*device->dct.eps));
    NVSHMEMI_NULL_ERROR_JMP(device->dct.eps, status,
                            NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                            "allocation of dct.eps failed.");

    device->dci.eps = (struct ibgda_ep **)calloc(device->dci.num_eps,
                                                 sizeof(*device->dci.eps));
    NVSHMEMI_NULL_ERROR_JMP(device->dci.eps, status,
                            NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                            "allocation of dci.eps failed.");

    device->rc.peer_ep_handles = (struct ibgda_rc_handle *)calloc(
        num_rc_eps, sizeof(*device->rc.peer_ep_handles));
    NVSHMEMI_NULL_ERROR_JMP(device->rc.peer_ep_handles, status,
                            NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                            "allocation of rc.peer_ep_handles failed.");

    device->rc.eps =
        (struct ibgda_ep **)calloc(num_rc_eps, sizeof(*device->dci.eps));
    NVSHMEMI_NULL_ERROR_JMP(device->rc.eps, status,
                            NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
    ...
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_create_qps: dct.num_eps = %d, dci.num_eps = %d, "
          "rc.num_eps_per_pe = %d",
          device->dct.num_eps, device->dci.num_eps, device->rc.num_eps_per_pe);
    /* create DCTs start */
    for (int i = 0; i < device->dct.num_eps; ++i) {
      status =
          ibgda_create_dct(ibgda_state, &device->dct.eps[i], device, portid);
      NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                            "ibgda_create_dct failed on DCT #%d.", i);

      status = ibgda_get_dct_handle(&local_dct_handles[i], device->dct.eps[i],
                                    device);
      NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "allgather of
    dct failed.");
    */
    ...
    INFO(ibgda_state->log_level,
         "Creating %d DCI QPs (shared: %d, exclusive: %d)", device->dci.num_eps,
         device->dci.num_shared_eps,
    ...
    for (int i = 0; i < device->dci.num_eps; ++i) {
      ...
      status =
          ibgda_dci_init2rtr(ibgda_state, device->dci.eps[i], device, portid);
      NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_create_qps: create_qps for device=%d, portid=%d, "
          "mype=%d.",
          curr_dev_id, portid, mype);

    /* create and assign RCs start */
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_create_qps: creating %d RC QPs",
    ...
    for (int i = 0; i < num_rc_eps; ++i) {
      ...
      status = ibgda_create_qp(&device->rc.eps[mapped_i], device, portid,
                               mapped_i, NVSHMEMI_IBGDA_DEVICE_QP_TYPE_RC);
      NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                            "ibgda_create_dci failed on RC #%d.", mapped_i);

      status = ibgda_get_rc_handle(&local_rc_handles[mapped_i],
  ...
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "alltoall of rc
  failed.");
  */
      print_guid(device->gid_info[portid - 1].local_gid.raw,
                 ibgda_state->log_level);
      *out_rc_handles = local_rc_handles;
      // top num_rc_eps_per_pe for pe = 0; bottom num_rc_eps_per_pe for pe = 1
      *out_handles_len = num_rc_eps * sizeof(*local_rc_handles);
      print_ibgda_rc_handle(local_rc_handles, num_rc_eps, mype,
                            ibgda_state->log_level);
    }
  }
...
out:
out_already_connected:
  // let caller deallocate the handles
  /*
  if (local_rc_handles) {
...
int nvshmemt_ibgda_connect_endpoints(nvshmem_transport_t t,
  ...
  /* Allocate global structs start */
  local_rc_handles = (struct ibgda_rc_handle *)rc_handles;
  NVSHMEMI_NULL_ERROR_JMP(local_rc_handles, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "outside rc_handles is NULL.\n");
  /* Allocate global structs end */
  ...
  for (int i = 0; i < num_selected_devs; i++) {
    ...
    TRACE(
        ibgda_state->log_level,
        "nvshmemt_ibgda_connect_endpoints: for device=%d, portid=%d, mype=%d.",
        curr_dev_id, portid, mype);

    /* create and assign RCs start */
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_connect_endpoints: creating %d RC QPs",
          device->rc.num_eps_per_pe);

    if (num_rc_eps) {
      memcpy((void *)device->rc.peer_ep_handles, (const void *)local_rc_handles,
             num_rc_eps * sizeof(*local_rc_handles));
      print_ibgda_rc_handle(local_rc_handles, num_rc_eps, mype,
                            ibgda_state->log_level);
    }
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_connect_endpoints: num_rc_eps=%d.", num_rc_eps);
    for (int i = 0; i < num_rc_eps; ++i) {
      // No loopback to self
      if (i / device->rc.num_eps_per_pe == mype) {
        continue;
      }
      TRACE(ibgda_state->log_level, "nvshmemt_ibgda_connect_endpoints: idx=%d.",
            i);
      status = ibgda_qp_rst2init(device->rc.eps[i], device, portid);
      NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                            "ibgda_qp_rst2init failed on RC #%d.", i);

      status = ibgda_rc_init2rtr(
          ibgda_state, device->rc.eps[i], device, portid,
          &device->rc.peer_ep_handles[(i + num_rc_eps_per_pe) % num_rc_eps]);
      NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
      ...
      print_ibgda_ep(device->rc.eps[i], i, ibgda_state->log_level);
    }
...
out:
  ...
  if (init_dev_cnt) {
    ...
    TRACE(ibgda_state->log_level,
          "nvshmemt_ibgda_connect_endpoints: n_devs_selected=%d",
          ibgda_state->n_devs_selected);
  ...
  if (init_dev_cnt < num_selected_devs) {
    NVSHMEMI_WARN_PRINT(
        "Failed to initialize all selected devices. Perf may be limited.");
  }
...
int nvshmemt_ibgda_release_mem_handle(nvshmem_mem_handle_t *mem_handle,
  ...
  struct ibgda_mem_handle *ibgda_mem_handle =
      (struct ibgda_mem_handle *)mem_handle;
  struct nvshmemt_ib_common_mem_handle *handle =
      (struct nvshmemt_ib_common_mem_handle *)&ibgda_mem_handle
          ->dev_mem_handles[0];
  if (handle->local_only) {
    ...
    for (auto it = ibgda_device_local_only_mhandles.begin();
      ...
      if (it->mhandle.start == (uint64_t)handle->buf) {
        ...
        if (position > 0)
          prev_mhandle_cache =
              &ibgda_device_local_only_mhandles.data()[position - 1];
    ...
    NVSHMEMI_NULL_ERROR_JMP(curr_mhandle_cache, status,
    ...
    if (prev_mhandle_cache) {
      ...
      mhandle_gpu_ptr =
          (void *)((uintptr_t)prev_mhandle_cache->dev_ptr +
                   offsetof(nvshmemi_ibgda_device_local_only_mhandle_t, next));
      status = cudaMemcpyAsync(mhandle_gpu_ptr,
                               (const void *)&prev_mhandle_cache->mhandle.next,
                               sizeof(prev_mhandle_cache->mhandle.next),
                               cudaMemcpyHostToDevice, ibgda_state->my_stream);
      NVSHMEMI_NE_ERROR_JMP(
          status, cudaSuccess, NVSHMEMX_ERROR_INTERNAL, out,
          "Setting local_only_mhandle in GPU memory failed.\n");
    ...
    ibgda_device_local_only_mhandles.erase(
        ibgda_device_local_only_mhandles.begin() + position);
  }

  // TODO: Clean up non-local-only mem_handle

  for (int i = 0; i < ibgda_state->n_devs_selected; i++) {
    handle = (struct nvshmemt_ib_common_mem_handle *)&ibgda_mem_handle
                 ->dev_mem_handles[i];
    status = nvshmemt_ib_common_release_mem_handle(
        &ftable, (nvshmem_mem_handle_t *)handle, ibgda_state->log_level);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemt_ibgda_finalize(nvshmem_transport_t transport) {
  ...
  for (int i = 0; i < ibgda_state->n_dev_ids; i++) {
    device =
        (struct ibgda_device *)ibgda_state->devices + ibgda_state->dev_ids[i];
    if (device->pd) {
  ...
  nvshmemt_ibv_ftable_fini(&ibv_handle, ibgda_state->log_level);
...
int nvshmemt_ibgda_add_device_remote_mem_handles(
  ...
  nvshmemt_ibgda_state_t *ibgda_state = (nvshmemt_ibgda_state_t *)t->state;
  TRACE(ibgda_state->log_level,
        "nvshmemt_ibgda_add_device_remote_mem_handles enter");
  ...
  static_assert(sizeof(struct nvshmemt_ib_common_mem_handle) <=
  ...
  TRACE(
      ibgda_state->log_level,
      "nvshmemt_ibgda_add_device_remote_mem_handles: num element=%ld, n_pes=%d",
      num_elements, n_pes);
  while (num_elements > 0) {
    // modify n_pes to 1 for now
    for (int i = 0; i < 1; ++i) {
      ...
      for (int j = 0; j < gmhandle->num_devs; j++) {
        ...
        TRACE(ibgda_state->log_level,
              "nvshmemt_ibgda_add_device_remote_mem_handles: i=%d, stride=%d, "
              "offset=%d, mem_handle rkey=%u, device rkey=%u",
              i, transport_stride, i * transport_stride + t->index,
              handle->rkey, device_key.key);
  ...
  memcpy(ibgda_device_state->constmem.rkeys, ibgda_device_rkeys.data(),
         IBGDA_MIN(num_rkeys, NVSHMEMI_IBGDA_MAX_CONST_RKEYS) *
             sizeof(nvshmemi_ibgda_device_key_t));

  // Put the rest that don't fit in constant memory in global memory
  if (num_rkeys > NVSHMEMI_IBGDA_MAX_CONST_RKEYS) {
    size_t rkeys_array_size = sizeof(nvshmemi_ibgda_device_key_t) *
                              (num_rkeys - NVSHMEMI_IBGDA_MAX_CONST_RKEYS);
    ...
    status = cudaMemcpyAsync(ibgda_device_rkeys_d, (const void *)data_ptr,
  ...
  ibgda_device_state->globalmem.rkeys =
      (nvshmemi_ibgda_device_key_t *)ibgda_device_rkeys_d;
out:
  if (status) {
    // Unrecoverable error
    if (ibgda_device_rkeys_d)
  ...
  TRACE(ibgda_state->log_level,
        "nvshmemt_ibgda_add_device_remote_mem_handles exit");
...
static int ibgda_check_nic_mapping_memtypes(
  ...
  bool try_gpumem =
      ((request_memtype == IBGDA_NIC_MAPPING_MEMTYPE_REQUEST_AUTO) ||
  ...
  if (try_gpumem) {
    status =
        ibgda_gpu_mem_alloc(&mobject, IBGDA_DBRSIZE, IBGDA_GPAGE_SIZE, false);
    if (status)
      goto out_try_gpumem;

    if (!ibgda_state->options->IB_DISABLE_DMABUF &&
        ibgda_state->cuda_support_dmabuf) {
      status = ibgda_mobject_nic_map(mobject, device->context,
  ...
  if (try_hostmem) {
    status =
        ibgda_host_mem_alloc(&mobject, IBGDA_DBRSIZE, IBGDA_GPAGE_SIZE, true);
    if (status)
      goto out_try_hostmem;

    status =
        ibgda_mobject_nic_map(mobject, device->context, IBV_ACCESS_LOCAL_WRITE);
    if (status)
      goto out_try_hostmem;
...
static int ibgda_check_gpu_mapping_nic_uar(struct ibgda_device *device) {
  ...
  status = ibgda_alloc_and_map_qp_uar(device->context, IBGDA_NIC_HANDLER_GPU,
                                      &mobject);
  if (status) {
    NVSHMEMI_WARN_PRINT("ibgda_alloc_and_map_qp_uar with GPU as handler "
                        "failed. We may need to enter the CPU "
                        "fallback path.\n");
    status = NVSHMEMX_ERROR_INTERNAL;
    goto out;
  }
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table,
  ...
  if (NVSHMEM_TRANSPORT_MAJOR_VERSION(api_version) !=
  ...
  NVSHMEMI_NULL_ERROR_JMP(
      options, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
  ...
  transport =
      (struct nvshmem_transport *)malloc(sizeof(struct nvshmem_transport));
  NVSHMEMI_NULL_ERROR_JMP(
      transport, status, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
  ...
  if (ibgda_num_requests_in_batch > 0) {
    ibgda_num_requests_in_batch =
        IBGDA_ROUND_UP_POW2_OR_0(ibgda_num_requests_in_batch);
  }
  if (ibgda_num_requests_in_batch <= 0) {
    NVSHMEMI_ERROR_JMP(
        status, NVSHMEMX_ERROR_INVALID_VALUE, out,
    ...
    NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INVALID_VALUE, out,
                       "NVSHMEM_IBGDA_NUM_REQUESTS_IN_BATCH must not be larger "
                       "than QP depth.\n");
  }

  ibgda_num_fetch_slots_per_dci = options->IBGDA_NUM_FETCH_SLOTS_PER_DCI;
  if (ibgda_num_fetch_slots_per_dci > 0) {
    ibgda_num_fetch_slots_per_dci =
        IBGDA_ROUND_UP_POW2_OR_0(ibgda_num_fetch_slots_per_dci);
  }
  if (ibgda_num_fetch_slots_per_dci <= 0) {
    NVSHMEMI_ERROR_JMP(
        status, NVSHMEMX_ERROR_INVALID_VALUE, out,
  ...
  if (ibgda_num_fetch_slots_per_rc > 0) {
    ibgda_num_fetch_slots_per_rc =
        IBGDA_ROUND_UP_POW2_OR_0(ibgda_num_fetch_slots_per_rc);
  }
  if (ibgda_num_fetch_slots_per_rc <= 0) {
    NVSHMEMI_ERROR_JMP(
        status, NVSHMEMX_ERROR_INVALID_VALUE, out,
  ...
  ibgda_state =
      (nvshmemt_ibgda_state_t *)calloc(1, sizeof(nvshmemt_ibgda_state_t));
  NVSHMEMI_NULL_ERROR_JMP(ibgda_state, status, NVSHMEMX_ERROR_OUT_OF_MEMORY,
  ...
  ibgda_state->dmabuf_support_for_data_buffers =
      ibgda_state->cuda_support_dmabuf;
  ...
  if (ibgda_state->dmabuf_support_for_data_buffers == false) {
    if (nvshmemt_ib_common_nv_peer_mem_available() != NVSHMEMX_SUCCESS) {
      NVSHMEMI_ERROR_PRINT("neither nv_peer_mem, or nvidia_peermem detected. "
                           "Skipping transport.\n");
      status = NVSHMEMX_ERROR_INTERNAL;
      goto out;
    }
  ...
  NVSHMEMI_NULL_ERROR_JMP(ibgda_state->devices, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "get_device_list failed \n");

  ibgda_state->dev_ids = (int *)malloc(MAX_NUM_PES_PER_NODE * sizeof(int));
  NVSHMEMI_NULL_ERROR_JMP(ibgda_state->dev_ids, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "malloc failed \n");

  ibgda_state->port_ids = (int *)malloc(MAX_NUM_PES_PER_NODE * sizeof(int));
  NVSHMEMI_NULL_ERROR_JMP(ibgda_state->port_ids, status,
                          NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                          "malloc failed \n");
  if (options->HCA_LIST_provided) {
    user_selection = 1;
    exclude_list = (options->HCA_LIST[0] == '^');
    hca_list_count = nvshmemt_parse_hca_list(
        options->HCA_LIST, hca_list, MAX_NUM_HCAS, ibgda_state->log_level);
  }

  if (options->HCA_PE_MAPPING_provided) {
    if (hca_list_count) {
      NVSHMEMI_WARN_PRINT("Found conflicting parameters NVSHMEM_HCA_LIST and "
  ...
  nic_mapping_memtype_request = ibgda_parse_nic_mapping_memtype_request(
      options->IBGDA_FORCE_NIC_BUF_MEMTYPE);
  ...
  if (nic_mapping_memtype_request != IBGDA_NIC_MAPPING_MEMTYPE_REQUEST_GPUMEM) {
    NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_NOT_SUPPORTED, out,
                       "GPU-initiated communication is compiled with GPU "
                       "memory support only.\n");
  }
#endif

  status = ibgda_parse_nic_handler_request(&nic_handler_request,
                                           options->IBGDA_NIC_HANDLER);
  NVSHMEMI_NZ_ERROR_JMP(status, status, out,
                        "NVSHMEM_IBGDA_NIC_HANDLER is not valid.");
  ...
  if (!use_gdrcopy) {
    if (nic_handler_request == IBGDA_NIC_HANDLER_AUTO) {
      ...
      NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_NOT_SUPPORTED, out,
                         "NVSHMEM_IBGDA_NIC_HANDLER=cpu is not supported.\n");
    }
  }

  INFO(ibgda_state->log_level, "Begin - Enumerating IB devices in the system "
                               "([<dev_id, device_name, num_ports>]) - \n");
  for (int i = 0; i < num_devices; i++) {
    ...
    if (!device->context) {
      INFO(ibgda_state->log_level,
           "open_device failed for IB device at index %d\n", i);
      continue;
    }
    ...
    if (nic_handler_request == IBGDA_NIC_HANDLER_CPU) {
      ...
      device->nic_handler =
          status ? IBGDA_NIC_HANDLER_CPU : IBGDA_NIC_HANDLER_GPU;
    ...
    status = ibgda_check_nic_mapping_memtypes(ibgda_state, device,
                                              nic_mapping_memtype_request);
    if (status) {
      ftable.close_device(device->context);
      device->context = NULL;
      NVSHMEMI_WARN_PRINT("device %s cannot allocate buffer on the specified "
                          "memory type. Skipping...\n",
                          name);
      continue;
    }

    if (device->support_nic_buf_on_gpumem &&
        !ibgda_state->options->IB_DISABLE_DMABUF &&
        !ibgda_state->dmabuf_support_for_control_buffers) {
      INFO(ibgda_state->log_level,
           "The system does not support registering the NIC control buffers "
           "with DMABUF. "
    ...
    if (status) {
      ...
      device->context = NULL;
      NVSHMEMI_WARN_PRINT(
          "device %s does not support all necessary atomic operations. You may "
          "want to check "
    ...
    INFO(ibgda_state->log_level,
         "Enumerated IB devices in the system - device id=%d (of %d), name=%s, "
         "num_ports=%d\n",
    ...
    for (int p = 1; p <= device->device_attr.phys_port_cnt; p++) {
      ...
      if (!allowed_device) {
        ...
        status =
            ftable.query_port(device->context, p, &device->port_attr[p - 1]);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                              "ibv_port_query failed \n");

        // IBGDA supports IB and RoCE.
        if ((device->port_attr[p - 1].state != IBV_PORT_ACTIVE) ||
            ((device->port_attr[p - 1].link_layer !=
              IBV_LINK_LAYER_INFINIBAND) &&
             (device->port_attr[p - 1].link_layer !=
              IBV_LINK_LAYER_ETHERNET))) {
          if (user_selection) {
            NVSHMEMI_WARN_PRINT("found inactive port or port with non IB/RoCE "
                                "link layer protocol, "
                                "skipping...\n");
          }
          continue;
        ...
        ib_get_gid_index(&ftable, device->context, p,
                         device->port_attr[p - 1].gid_tbl_len,
                         &device->gid_info[p - 1].local_gid_index,
                         ibgda_state->log_level, options);
        status = ftable.query_gid(device->context, p,
                                  device->gid_info[p - 1].local_gid_index,
        ...
        if (!device->pd) {
          ...
          NVSHMEMI_NULL_ERROR_JMP(device->pd, status, NVSHMEMX_ERROR_INTERNAL,
                                  out, "ibv_alloc_pd failed \n");
        }
    ...
    /* Report whether we need to do atomic endianness conversions on 8 byte
     * operands. */
    status = nvshmemt_ib_common_query_endianness_conversion_size(
        &atomic_host_endian_size, device->context);
    NVSHMEMI_NZ_ERROR_JMP(
        status, NVSHMEMX_ERROR_INTERNAL, out,
  ...
  INFO(ibgda_state->log_level, "Begin - Ordered list of devices for assignment "
                               "(after processing user provdied env vars "
                               "(if any))  - \n");
  for (int i = 0; i < ibgda_state->n_dev_ids; i++) {
    INFO(ibgda_state->log_level,
         "Ordered list of devices for assignment - idx=%d (of %d), device "
         "id=%d, port_num=%d\n",
         i, ibgda_state->n_dev_ids, ibgda_state->dev_ids[i],
         ibgda_state->port_ids[i]);

    device =
        (struct ibgda_device *)ibgda_state->devices + ibgda_state->dev_ids[i];
    nic_buf_on_gpumem &= device->support_nic_buf_on_gpumem;
    nic_buf_on_hostmem &= device->support_nic_buf_on_hostmem;
    if (device->nic_handler == IBGDA_NIC_HANDLER_CPU)
      nic_handler = IBGDA_NIC_HANDLER_CPU;
  }
  INFO(ibgda_state->log_level, "End - Ordered list of devices for assignment "
                               "(after processing user provdied env vars "
                               "(if any))\n");

  if (!ibgda_state->n_dev_ids) {
    INFO(ibgda_state->log_level,
         "no active IB device that supports GPU-initiated communication is "
         "found, exiting...\n");
    status = NVSHMEMX_ERROR_INTERNAL;
    goto out;
  }
  ...
  transport->device_pci_paths =
      (char **)calloc(transport->n_devices, sizeof(char *));
  NVSHMEMI_NULL_ERROR_JMP(transport->device_pci_paths, status,
  ...
  status = cudaDeviceGetStreamPriorityRange(&lowest_stream_priority,
                                            &highest_stream_priority);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "stream priority query failed. \n");
  status = cudaStreamCreateWithPriority(
      &ibgda_state->my_stream, cudaStreamNonBlocking, highest_stream_priority);
  NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                        "internal stream creation failed. \n");

  // we do not support gdrcopy with CPU nic handler
  assert(use_gdrcopy == false && (nic_handler == IBGDA_NIC_HANDLER_GPU));
  transport->host_ops.can_reach_peer = nvshmemt_ibgda_can_reach_peer;
  transport->host_ops.create_qps = nvshmemt_ibgda_create_qps;
  transport->host_ops.connect_endpoints = nvshmemt_ibgda_connect_endpoints;
  // change default get_mem_handle to venus ibgda_get_mem_handle_v2
  transport->host_ops.get_mem_handle = nvshmemt_ibgda_get_mem_handle_v2;
