...
NVSHMEMI_ENV_DEF(DISABLE_IB_NATIVE_ATOMICS, bool, false,
                 NVSHMEMI_ENV_CAT_TRANSPORT,
...
NVSHMEMI_ENV_DEF(IB_GID_INDEX, int, -1, NVSHMEMI_ENV_CAT_TRANSPORT,
                 "Source GID Index for ROCE")
NVSHMEMI_ENV_DEF(IB_TRAFFIC_CLASS, int, 0, NVSHMEMI_ENV_CAT_TRANSPORT,
                 "Traffic calss for ROCE")
NVSHMEMI_ENV_DEF(
    IB_ADDR_FAMILY, string, "AF_INET", NVSHMEMI_ENV_CAT_TRANSPORT,
    "IP address family associated to IB GID "
    "dynamically selected by NVSHMEM when NVSHMEM_IB_GID_INDEX is left unset")
NVSHMEMI_ENV_DEF(IB_ADDR_RANGE, string, "::/0", NVSHMEMI_ENV_CAT_TRANSPORT,
                 "Defines the range of "
                 "valid GIDs dynamically selected by NVSHMEM when "
                 "NVSHMEM_IB_GID_INDEX is left unset")
NVSHMEMI_ENV_DEF(
    IB_ROCE_VERSION_NUM, int, 2, NVSHMEMI_ENV_CAT_TRANSPORT,
...
NVSHMEMI_ENV_DEF(IB_ENABLE_RELAXED_ORDERING, bool, true,
                 NVSHMEMI_ENV_CAT_TRANSPORT,
                 "Enable PCIe relaxed ordering on transports over IB/ROCE "
                 "(e.g., IBRC, IBGDA, IBDEVX)")

NVSHMEMI_ENV_DEF(
    HCA_LIST, string, "", NVSHMEMI_ENV_CAT_TRANSPORT,
    ...
    "prefixed by ^ are excluded. ``NVSHMEM_ENABLE_NIC_PE_MAPPING`` must be set "
    "to "
    "1 for this variable to be effective.")

NVSHMEMI_ENV_DEF(
    HCA_PE_MAPPING, string, "", NVSHMEMI_ENV_CAT_TRANSPORT,
    ...
    "``NVSHMEM_ENABLE_NIC_PE_MAPPING`` must be set to 1 for this variable to "
    "be "
    "effective.")
...
NVSHMEMI_ENV_DEF(IBGDA_NUM_SHARED_DCI, int, 1, NVSHMEMI_ENV_CAT_TRANSPORT,
                 ...
                 "The rest of DCI QPs (NVSHMEM_IBGDA_NUM_DCI - "
                 "NVSHMEM_IBGDA_NUM_SHARED_DCI) are "
...
NVSHMEMI_ENV_DEF(IBGDA_NUM_RC_PER_PE, int, 2, NVSHMEMI_ENV_CAT_TRANSPORT,
                 "Number of RC QPs per peer PE used in GPU-initiated "
                 "communication transport. "
                 "Set to 0 to disable RC QPs (default 2). "
                 "If set to a positive number, DCI will be used for enforcing "
                 "consistency only.")
NVSHMEMI_ENV_DEF(IBGDA_RC_MAP_BY, string, "cta", NVSHMEMI_ENV_CAT_TRANSPORT,
                 ...
                 "- sm: round-robin by SM ID.\n"
                 "- warp: round-robin by Warp ID.")
NVSHMEMI_ENV_DEF(IBGDA_FORCE_NIC_BUF_MEMTYPE, string, "gpumem",
                 NVSHMEMI_ENV_CAT_TRANSPORT,
                 "Force NIC buffer memory type. Valid choices are: gpumem "
                 "(default), hostmem. "
                 "For other values, use auto discovery.")
NVSHMEMI_ENV_DEF(
    IBGDA_NUM_REQUESTS_IN_BATCH, int, 32, NVSHMEMI_ENV_CAT_TRANSPORT,
...
NVSHMEMI_ENV_DEF(
    IBGDA_NUM_FETCH_SLOTS_PER_DCI, int, 1024, NVSHMEMI_ENV_CAT_TRANSPORT,
    "Number of internal buffer slots for fetch operations for each DCI QP. "
    "It will be rounded up to the nearest power of 2.")
NVSHMEMI_ENV_DEF(
    IBGDA_NUM_FETCH_SLOTS_PER_RC, int, 1024, NVSHMEMI_ENV_CAT_TRANSPORT,
...
NVSHMEMI_ENV_DEF(IBGDA_NIC_HANDLER, string, "auto", NVSHMEMI_ENV_CAT_TRANSPORT,
                 ...
                 "- auto: use GPU SMs and fallback to CPU if it is not "
                 "supported (default).\n"
                 "- gpu: use GPU SMs.\n"
                 "- cpu: use CPU.")
