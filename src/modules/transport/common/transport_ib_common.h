...
inline void print_nvshmemt_ib_common_mem_handle(
    struct nvshmemt_ib_common_mem_handle *handle, int log_level) {
  TRACE(log_level,
        "print_nvshmemt_ib_common_mem_handle: mr=%p, buf=%p, fd=%d, lkey=%u, "
        "rkey=%u, local_only=%d.",
        handle->mr, handle->buf, handle->fd, handle->lkey, handle->rkey,
        handle->local_only);
  return;
}
...
/* The following code is for dynamic GID detection for RoCE platforms.
   ...
   https://gitlab-master.nvidia.com/nccl/nccl/-/merge_requests/359 */
static sa_family_t env_ib_addr_family(int log_level,
...
static void *env_ib_addr_range(sa_family_t af, int *prefix_len, int log_level,
  ...
  *(mask_str_ptr - 1) = '\0';
  ...
  if (inet_pton(af, addr_str_ptr, ret) == 0) {
    INFO(log_level,
         "NET/IB: Ip address '%s' is invalid for family %s, ignoring address",
  ...
  *prefix_len = (int)strtol(mask_str_ptr, NULL, 10);
  if (af == AF_INET && *prefix_len > 32) {
    INFO(log_level,
         "IB: Ip address mask '%d' is invalid for family %s, ignoring mask",
...
static sa_family_t get_gid_addr_family(union ibv_gid *gid) {
  ...
  bool is_ipv4_mapped = ((a->s6_addr32[0] | a->s6_addr32[1]) |
...
static bool valid_gid(union ibv_gid *gid) {
  return (configured_gid(gid) && !link_local_gid(gid));
}

static int ib_roce_get_version_num(const char *deviceName, int portNum,
  ...
  if (fd == -1) {
    NVSHMEMI_WARN_PRINT("IB: open failed in ib_roce_get_version_num: %s",
                        strerror(errno));
  ...
  if (ret == -1) {
    NVSHMEMI_WARN_PRINT("IB: read failed in ib_roce_get_version_num: %s",
                        strerror(errno));
    return NVSHMEMX_ERROR_INTERNAL;
  }
...
static void update_gid_index(struct nvshmemt_ibv_function_table *ftable,
  ...
  bool gidCandidateMatchSubnet =
      match_gid_addr_prefix(usrFam, prefix, prefixlen, &gidCandidate);

  if (gidCandidateFam != gidFam && gidCandidateFam == usrFam &&
      gidCandidateMatchSubnet) {
    *gidIndex = gidIndexCandidate;
  } else {
    if (gidCandidateFam != usrFam || !valid_gid(&gidCandidate) ||
        !gidCandidateMatchSubnet) {
      return;
    }
    ...
    ib_roce_get_version_num(deviceName, portNum, gidIndexCandidate,
...
static void ib_get_gid_index(struct nvshmemt_ibv_function_table *ftable,
                             struct ibv_context *context, uint8_t portNum,
                             int gidTblLen, int *gidIndex, int log_level,
                             nvshmemi_options_s *options) {
  *gidIndex = options->IB_GID_INDEX;
  ...
  int prefixlen;
  void *prefix =
      env_ib_addr_range(userAddrFamily, &prefixlen, log_level, options);

  *gidIndex = 0;
  for (int gidIndexNext = 1; gidIndexNext < gidTblLen; ++gidIndexNext) {
    update_gid_index(ftable, context, portNum, userAddrFamily, prefix,
