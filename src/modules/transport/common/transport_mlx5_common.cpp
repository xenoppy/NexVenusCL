...
int nvshmemt_ib_common_query_endianness_conversion_size(
    uint32_t *endianness_mode, struct ibv_context *context) {
  void *cap;
  ...
  amo_endianness_mode =
      DEVX_GET(atomic_caps, cap, atomic_req_8B_endianness_mode);
...
int nvshmemt_ib_common_check_nic_ext_atomic_support(
  ...
  if (!(atomic_operations & MLX5_ATOMIC_CAP_OP_SUPPORT_MASKED_CAS)) {
    NVSHMEMI_WARN_PRINT(
        "device does not support atomic masked compared and swap\n");
    status = NVSHMEMX_ERROR_NOT_SUPPORTED;
    goto out;
  }
  ...
  if (!(atomic_operations & MLX5_ATOMIC_CAP_OP_SUPPORT_MASKED_FA)) {
    NVSHMEMI_WARN_PRINT(
        "device does not support atomic masked fetch and add\n");
    status = NVSHMEMX_ERROR_NOT_SUPPORTED;
    goto out;
  }
