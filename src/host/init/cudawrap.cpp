...
#define LOAD_SYM(table, symbol, version, sym_suffix, ignore)                   \
  do {                                                                         \
    ...
    if (table->pfn_cuGetProcAddress) {                                         \
      res = table->pfn_cuGetProcAddress(                                       \
          #symbol, (void **)(&table->pfn_##symbol), version, 0);               \
      if (res != 0)                                                            \
        not_found = true;                                                      \
    } else {                                                                   \
      table->pfn_##symbol =                                                    \
          (PFN_##symbol##_v##version)dlsym(cudaLib, #symbol #sym_suffix);      \
      if (table->pfn_##symbol == NULL)                                         \
        not_found = true;                                                      \
    }                                                                          \
    if (not_found) {                                                           \
      if (!ignore) {                                                           \
        WARN("Retrieve %s version %d failed", #symbol #sym_suffix,             \
             cudaDriverVersion);                                               \
...
int nvshmemi_cuda_library_init(struct nvshmemi_cuda_fn_table *table) {
  ...
  table->pfn_cuGetProcAddress =
      (PFN_cuGetProcAddress_v11030)dlsym(cudaLib, "cuGetProcAddress");
