...
void nvshmem_debug_log(nvshmem_debug_log_level level, unsigned long flags,
  ...
  if (level == NVSHMEM_LOG_WARN && nvshmem_debug_level >= NVSHMEM_LOG_WARN)
    len = snprintf(buffer, sizeof(buffer),
                   "\n%s:%d:%d [%d] %s:%d NVSHMEM WARN ", hostname, getpid(),
                   nvshmemi_gettid(), cudaDev, filefunc, line);
  else if (level == NVSHMEM_LOG_INFO &&
           nvshmem_debug_level >= NVSHMEM_LOG_INFO &&
           (flags & nvshmem_debug_mask))
    len = snprintf(buffer, sizeof(buffer), "%s:%d:%d [%d] NVSHMEM INFO ",
                   hostname, getpid(), nvshmemi_gettid(), cudaDev);
#ifdef NVSHMEM_TRACE
  else if (level == NVSHMEM_LOG_TRACE &&
    ...
    double timestamp =
        ...
        1000;
    len = snprintf(buffer, sizeof(buffer),
                   "%s:%d:%d [%d] %f %s:%d NVSHMEM TRACE ", hostname, getpid(),
                   nvshmemi_gettid(), cudaDev, timestamp, filefunc, line);
  }
#endif
  if (len) {
    va_list vargs;
  ...
  // If nvshmem_debug_level == NVSHMEM_LOG_ABORT then WARN() will also call
  // abort()
