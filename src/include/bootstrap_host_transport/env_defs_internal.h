...
static inline int nvshmemi_atol_scaled(const char *str,
                                       nvshmemi_env_size *out) {
  int scale, n;
  double p = -1.0;
  char f;
...
static inline const char *nvshmemi_getenv_helper(const char *prefix,
                                                 const char *name) {
  char *env_name;
...
static inline int nvshmemi_getenv_string(const char *name,
                                         nvshmemi_env_string default_val,
                                         nvshmemi_env_string *out,
                                         bool *provided) {
...
static inline int nvshmemi_getenv_int(const char *name,
                                      nvshmemi_env_int default_val,
...
static inline int nvshmemi_getenv_long(const char *name,
                                       nvshmemi_env_long default_val,
...
static inline int nvshmemi_getenv_size(const char *name,
                                       nvshmemi_env_size default_val,
...
static inline int nvshmemi_getenv_bool(const char *name,
                                       nvshmemi_env_bool default_val,
                                       nvshmemi_env_bool *out, bool *provided) {
  const char *env = nvshmemi_getenv(name);
  *provided = (env != NULL);

  if (*provided && (env[0] == '0' || env[0] == 'N' || env[0] == 'n' ||
                    env[0] == 'F' || env[0] == 'f')) {
    *out = false;
  } else if (*provided) {
...
static inline int
nvshmemi_env_options_init(struct nvshmemi_options_s *options) {
  int ret;
#define NVSHMEMI_ENV_DEF(NAME, KIND, DEFAULT, CATEGORY, SHORT_DESC)            \
  ret = nvshmemi_getenv_##KIND(#NAME, DEFAULT, &(options->NAME),               \
                               &(options->NAME##_provided));                   \
  if (ret)                                                                     \
    return ret;
#include "env_defs.h"
#undef NVSHMEMI_ENV_DEF
