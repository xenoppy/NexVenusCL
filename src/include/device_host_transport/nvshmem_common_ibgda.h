...
#define nvshmemi_init_ibgda_device_qp_management(qp_man)                       \
  do {                                                                         \
    qp_man.version =                                                           \
        (1 << 16) + sizeof(nvshmemi_ibgda_device_qp_management_t);             \
...
#define nvshmemi_init_ibgda_device_local_only_memhandle(mhandle)               \
  do {                                                                         \
    mhandle.version =                                                          \
        (1 << 16) + sizeof(nvshmemi_ibgda_device_local_only_mhandle_t);        \
...
static_assert(sizeof(nvshmemi_ibgda_device_cq_v1) == 72,
              "ibgda_device_cq_v1 must be 72 bytes.");
...
typedef struct {
  ...
  struct {
    ...
    uint64_t get_head;   // last wqe idx + 1 with a "fetch" operation (g, get,
                         // amo_fetch)
    uint64_t get_tail; // last wqe idx + 1 polled with cst; get_tail > get_head
                       // is possible
  } tx_wq;
  struct {
    uint64_t resv_head; // last reserved wqe idx + 1
...
typedef nvshmemi_ibgda_device_qp_management_v1
...
static_assert(sizeof(nvshmemi_ibgda_device_qp_v1) == 256,
              "ibgda_device_qp_v1 must be 248 bytes.");
...
typedef nvshmemi_ibgda_device_local_only_mhandle_v1
...
typedef struct {
  ...
  struct {
    ...
    nvshmemi_ibgda_device_cq_t
        *cqs; // For both dcis and rcs. CQs for DCIs come first.
    ...
    // rkeys[(idx * npes + pe) - NVSHMEMI_IBGDA_MAX_CONST_RKEYS] gives rkey of
    // chunck idx targeting peer pe.
...
// Infrawaves: Simplied version of ibgda_device_state_t
typedef struct {
  int version;
  uint32_t num_rc_per_pe;
  nvshmemi_ibgda_device_qp_map_type_t rc_map_type;
  int num_devices_initialized;
  nvshmemi_ibgda_device_key_t lkey;
  nvshmemi_ibgda_device_key_t rkey;
  nvshmemi_ibgda_device_cq_t *cqs;
  nvshmemi_ibgda_device_qp_t *rcs;
} p2pcomm_ibgda_device_state_v1;
/*
static_assert(sizeof(p2pcomm_ibgda_device_state_v1) == 8384,
              "ibgda_device_state_v1 must be 8384 bytes.");
*/
typedef p2pcomm_ibgda_device_state_v1 p2pcomm_ibgda_device_state_t;
